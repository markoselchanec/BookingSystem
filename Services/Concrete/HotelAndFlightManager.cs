...
{
    public class HotelAndFlightManager : IManager
    {
        private readonly HttpClient _httpClient;
        private readonly IMemoryStore _memoryStore;

        public HotelAndFlightManager(HttpClient httpClient, IMemoryStore memoryStore)
        {
        ...
        public async Task<SearchResponse> Search(SearchRequest request)
        {
            ...
            var flightsUrl = $"https://tripx-test-functions.azurewebsites.net/api/SearchFlights?departureAirport={request.DepartureAirport}&arrivalAirport={request.Destination}";
            var flightsResponse = await _httpClient.GetStringAsync(flightsUrl);
            var flights = JsonSerializer.Deserialize<List<Flight>>(flightsResponse);

            if (flights is null || flights.Count is 0)
            {
                return response;
            }
